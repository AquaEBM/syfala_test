...
The `CFPluginFactories` entry must contain a dictionnary mapping unique UUIDs, _generated by the developer of the Plug-In_, in string form, (see $FACTORY_UUID) to symbol names exported by your library. Those symbols are the _Plug-In factories_ of your library. They are CoreAudio's entry point to your driver, and must always be declared in this file. The symbols you declare in this section are the only symbols your library needs to expose and you may give them whichever name you wish. It is important to note that, when compiled, when compiled, public symbols of a shared/dynamic libraries are often prefixed with and underscore (`_`). This means that the names declared here are not necessarily equal to the name of the corresponding functions defined in your source code. To check them you can run the following command:
